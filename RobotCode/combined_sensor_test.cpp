...
namespace {
constexpr int LINE_PIN = 35;
constexpr char LINE_LABEL[] = "FF";
...
void setup() {
  ...
  Serial.println("Combined sensor test ready");
  Serial.print("Line sensor ");
  Serial.print(LINE_LABEL);
  Serial.print(" on GPIO ");
  Serial.println(LINE_PIN);
  Serial.println("Colour output keeps the last non-black/non-white marker");
...
void loop() {
  ...
  const bool onLine = digitalRead(LINE_PIN) == HIGH;

  if (rgbSensorReady) {
    RgbReading reading = {};
    if (readRgbSensor(reading)) {
      const COLOUR detected = classifyColour(reading);
      if (detected != BLACK && detected != WHITE) {
        lastMarkerColour = detected;
      }
    }
  }
  ...
  Serial.print(" lastColour=");
  Serial.println(colourToString(lastMarkerColour));
}
